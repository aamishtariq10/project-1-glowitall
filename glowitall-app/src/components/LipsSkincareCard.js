...
const LipsSkincareCard = (props) => {
  
  const { grid } = props;
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const location = useLocation();
  const products = useSelector(state => state.lipsSkincareProduct.product);

  useEffect(() => {
    dispatch(fetchAllProductsFromLipsSkincare());
    console.log(fetchAllProductsFromLipsSkincare);
  }, [dispatch]);
...
export default LipsSkincareCard;
